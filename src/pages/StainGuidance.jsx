...
import {
  Droplet, Search, Camera, Loader2, CheckCircle2, Package,
  AlertCircle, Sparkles, Save, Check, Shirt, ChevronDown, Upload, RotateCcw, ArrowLeft
} from "lucide-react";
...
const STAIN_DB = [
  ...
  { name: "Chocolate",  emoji: "🍫", category: "Food & Oil", color: "bg-yellow-50 text-yellow-900 border-yellow-200" },
...
export default function StainGuidance() {
  ...
  const buildPrompt = (stain, supplyNames, fromPhoto) => {
    ...
    const fabricContext = fabricType
      ? `The fabric is: "${fabricType}". Tailor ALL instructions specifically for this fabric — adjust temperature, agitation, and chemical recommendations accordingly.`
      : "";
    const photoContext = fromPhoto ? "The user has uploaded a photo of the stain — use it to identify stain type, severity (fresh/set-in/dried), and fabric if possible." : "";
    return `You are a laundry expert. ${stainContext} ${fabricContext} ${photoContext}
...
- severity: "fresh" | "set-in" | "dried" (estimate based on image or description)
- confidence: "high" | "medium" | "low"
- success_likelihood: "high" | "medium" | "low" (chance of full removal)
- success_note: string (brief explanation of the likelihood)
...
Be concise, calm, and practical.`;
  ...
  const likelihoodColor = { high: "bg-green-100 text-green-700 border-green-200", medium: "bg-yellow-100 text-yellow-700 border-yellow-200", low: "bg-red-100 text-red-700 border-red-200" };
  const likelihoodLabel = { high: "High Success Rate", medium: "Medium Success Rate", low: "Difficult to Remove" };
  const severityColor = { fresh: "bg-blue-100 text-blue-700", "set-in": "bg-orange-100 text-orange-700", dried: "bg-red-100 text-red-700" };
      ...
      <div className="max-w-lg mx-auto px-5 pt-8">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-2xl font-semibold tracking-tight">Smart Stain Assistant</h1>
          </div>
          <p className="text-sm text-muted-foreground">
            Describe your stain or upload a photo for AI-powered removal steps
          </p>
        </motion.div>

        {!isPremium && (
          <div className="mt-10 rounded-2xl border border-border bg-card">
            <AIPremiumLock featureName="Smart Stain Assistant" />
          </div>
        )}
        ...
        {isPremium && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="mt-6 space-y-4">
            ...
            {/* Mode Toggle */}
            <div role="tablist" aria-label="Input mode" className="flex bg-muted rounded-xl p-1 gap-1">
              <button
                role="tab"
                aria-selected={mode === "text"}
                onClick={() => { setMode("text"); handleReset(); }}
                className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-all ${mode === "text" ? "bg-white shadow-sm text-foreground" : "text-muted-foreground"}`}
              >
                <Search className="w-4 h-4" aria-hidden="true" /> Describe Stain
              </button>
              <button
                role="tab"
                aria-selected={mode === "photo"}
                onClick={() => { setMode("photo"); handleReset(); }}
                className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-all ${mode === "photo" ? "bg-white shadow-sm text-foreground" : "text-muted-foreground"}`}
              >
                <Camera className="w-4 h-4" aria-hidden="true" /> Scan Photo
              </button>
            </div>
                        ...
                        <div className="mt-1 border border-border rounded-xl overflow-hidden max-h-48 overflow-y-auto">
                          {closetItems.map(item => (
                            <button key={item.id} onClick={() => handleSelectClosetItem(item)}
                              className={`w-full text-left px-3 py-2.5 text-sm hover:bg-secondary/60 transition-colors border-b border-border/50 last:border-0 ${selectedClosetItem?.id === item.id ? "bg-primary/5 text-primary font-medium" : ""}`}>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2 flex-wrap">
                    <Button size="sm" variant={activeCategory === null ? "default" : "outline"} className="rounded-full text-xs h-7" onClick={() => setActiveCategory(null)}>All</Button>
                    {CATEGORIES.map(cat => (
                      <Button key={cat} size="sm" variant={activeCategory === cat ? "default" : "outline"} className="rounded-full text-xs h-7" onClick={() => setActiveCategory(cat)}>{cat}</Button>
                    ))}
                  </div>
                ...
                {!previewUrl ? (
                  <div className="p-8 flex flex-col items-center gap-5">
                    ...
                    <div className="text-center">
                      <p className="font-medium">Scan a stained garment</p>
                      <p className="text-sm text-muted-foreground mt-1">Take a clear, close-up photo of the stain for best results.</p>
                    </div>
                    <div className="flex flex-col gap-3 w-full">
                      <label className="w-full">
                        <Button className="w-full gap-2 pointer-events-none">
                  ...
                  <div>
                    <img src={previewUrl} alt="Stain preview" className="w-full max-h-56 object-cover" />
                  ...
                  <Card className="p-4 border-0 shadow-sm bg-primary/5">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex flex-col gap-1 items-end">
                        ...
                        {guidance.severity && (
                          <Badge className={`text-xs capitalize ${severityColor[guidance.severity] || "bg-muted text-muted-foreground"}`}>
                            {guidance.severity === "set-in" ? "Set-In" : guidance.severity.charAt(0).toUpperCase() + guidance.severity.slice(1)} Stain
                          </Badge>
                        )}
                      </div>
                    </div>
                    {guidance.success_likelihood && (
                      <div className={`mt-3 flex items-center gap-2 text-xs font-medium px-3 py-2 rounded-lg border ${likelihoodColor[guidance.success_likelihood] || "bg-muted"}`}>
                        <CheckCircle2 className="w-3.5 h-3.5 shrink-0" />
                        <span>{likelihoodLabel[guidance.success_likelihood]}</span>
                        {guidance.success_note && <span className="font-normal">— {guidance.success_note}</span>}
                      </div>
                    )}
                    {previewUrl && (
